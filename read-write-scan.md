...
The server will support GET, PUT and DELETE for paths of the form `/<table>/<key>`. Let&#700;s PUT a value for `apple`.
...
The `Condition-TxClock` header also works with the HTTP DELETE method. The row will be deleted only if it has not been changed since the precondition time.
...
[ {"table": "fruit", "key": "apple", "op": "update", "obj": "granny smith"},
  {"table": "fruit", "key": "grape", "op": "update", "obj": "cabernet"} ]
EOF
<span class="go">HTTP/1.1 200 OK
...
An array of JSON objects represents the batch. Each element of the array names the table and key to affect, and an operation to perform on that key. The operations `create` and `update` are similar, except that `create` requires that the row is currently deleted, whereas `update` will apply the change whether the row currently exists or not. The operation `delete` should be clear enough. Finally, `hold` allows a row to participate in the precondition without effecting a change.
